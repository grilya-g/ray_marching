...
def f_op_union_i(v1, v2):
    """Makes objects v1 and v2 united
    :param v1: vec2, first object
    :param v2: vec2, second object
    :return vec2 object (first or second) which distance is smaller
    """
...
def sd_snowmen(p1, R1, Rcoef):
    """Draws snowmen
    :param p1: vec3, center of the bottom sphere of snowmen
    :param R1: ti.f32, radius of the bottom sphere
    :param Rcoef: ti.f32, relativity between radiuses of spheres
    :return ti.f32 distance to the snowmen
    """
    R2 = Rcoef * R1
    R3 = Rcoef * R2
...
def dist_vector(p):
    ...
    d5 = sd_snowmen(p - vec3(5., 2.5, -5.) / 3, 0.76, 0.8)
    # d5 = sd_trapezoid(p - vec3(5., 1.5, -5.) / 3, 5., 5., 1.)
    #            0   1
    # return vec5(d0, d1, d2, d3, d4)
    return vec6(d0, d1, d2, d3, d4, d5)
